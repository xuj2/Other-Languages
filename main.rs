...
fn main() {
    ...
    let t2 = thread::spawn(move || map_data(&xs_2));
    ...
    let mut int_sums : Vec<usize> = Vec::new();
    for i in 0..num_partitions{
        let xs2_i = xs2[i].clone();
        let t = thread::spawn(move || map_data(&xs2_i));
        let r = t.join().unwrap();
        int_sums.push(r);
    }
    println!("Intermediate sums = {:?}", int_sums);
    let sum2 = reduce_data(&int_sums);
    println!("Sum = {}", sum2);
}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut partition_extra = v.len() % num_partitions;
    ...
    for _i in 0..num_partitions{
        ...
        if partition_extra > 0{
            x.push(v[index]);
            index = index + 1;
            partition_extra = partition_extra - 1;
        }
        xs.push(x);
    }
    xs
